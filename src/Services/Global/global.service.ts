...
export default class GLobalService {
  public async editProfile(req: any, next: NextFunction): Promise<IUser | void> {
    const user = req.user;
    const { companyName, fullName, position, phoneNumber, address, numberOfStaff, location } =
      req.body;
    ...
    const result = await userRepository.findUserByIdAndUpdate(user._id, payload);
    if (result) {
      return result as IUser;
    }
    return next(new AppError("Failed update, Try again", statusCode.conflict()));
  }

  public async editPassword(req: any, next: NextFunction): Promise<IUser | void> {
    const user = req.user;
  ...
  public async editEmail(req: any, next: NextFunction): Promise<IUser | void> {
    ...
    return next(new AppError("Email update failed, Try again", statusCode.conflict()));
  }
